...
def time_series_to_tensor(list_series):
    ...
    pos_tensor = torch.Tensor(
        [
            [[planet.iloc[time][pos] for pos in ["X (km)", "Y (km)", "Z (km)"]] for planet in list_series]
            for time in range(len(list_series[0]))
        ]
    )
    vel_tensor = torch.Tensor(
        [
            [[planet.iloc[time][vel] for vel in ["VX (km/s)", "VY (km/s)", "VZ (km/s)"]] for planet in list_series]
            for time in range(len(list_series[0]))
        ]
    )
